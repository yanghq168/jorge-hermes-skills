...
| 2026-09-24 | 31 | Case R — agent-mode crons need exactly ONE run (Lesson 1) + credential-path trap (Lesson 2). Lesson 1 refines Case Q's "run at most once" rule: on agent-mode prompt crons the agent IS the canonical run, so it must execute the script ONCE to produce today's backup; on classic-cron / no_agent script-mode crons the backup already exists and the agent must run ZERO times. The wrong scheduler-mode assumption wastes a run (producing a duplicate backup) or skips a needed run (producing no backup at all). Lesson 2: when hunting for the SMTP credential on a Hermes cron, read `~/.hermes/cron/scripts/config_loader.py` FIRST (one call shows the exact YAML key path or env var name) rather than reading `config.yaml` directly and grepping the script for `smtp_pass`. On this deployment the latter pair returned `mail: null` and only the local-fallback string — the real credential path is in `config_loader.get_mail_config()`. |
