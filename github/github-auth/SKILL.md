...
| Push rejected by GitHub with "Push cannot contain secrets" | **GitHub Secret Scanning is blocking the push** — it detected a GitHub PAT (classic or fine-grained) inside committed files. This commonly happens when backing up session files, logs, or memory dumps that contain API call metadata with embedded tokens. Fix: (1) Sanitize the data before commit — redact or remove token strings from session JSON files, (2) If you must back up sessions, use a **dedicated private repository** with secret scanning disabled, or (3) Never commit raw session files to a public-facing repo. |
