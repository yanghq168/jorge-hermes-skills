...
## Aliyun ECS / AWS EC2 Equivalents

| Provider | Where to look | Key notes |
|---|---|---|
| **Aliyun ECS** | Console → ECS → Instances → Security Groups → Inbound Rules | Same three-layer model. "安全组" = security group. |
| **AWS EC2** | EC2 → Security Groups → Inbound Rules | "Security Groups" panel. VPC network ACLs are a fourth layer, but rarely the culprit for "port not reachable" cases. |
| **DigitalOcean** | Networking → Firewalls → Inbound Rules | Simpler — no host firewall surprise layer. |
| **Hetzner Cloud** | Firewalls → Inbound Rules | Same. |

The diagnostic recipe (`nc -zv <server-ip> <port>` from outside) works the same way for all of them.
